...
let make = (~isAutoPlaying: bool, ~onToggleAutoPlay: unit => unit, ~onReset: unit => unit) => {
  <div className="flex items-center gap-2 shrink-0">
    ...
    <button
      ...
      className={"px-3.5 py-2 sm:px-4 sm:py-2.5 rounded-md font-bold text-xs sm:text-sm whitespace-nowrap transition-all active:scale-95 cursor-pointer shadow-xs " ++ if (
        isAutoPlaying
      ) {
